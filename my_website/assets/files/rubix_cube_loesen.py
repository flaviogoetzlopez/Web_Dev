...
def transpose_matrix(array_to_rotate_coordinates_cube):
    """Transposes matrix, by inverting the indexes of all pieces in the cube. """
    """dafuer haette ich auch numpy's built in methode benutzen koennen, aber vom prinzip habe ich es nicht gemacht. numpy.rot90"""

    array_to_rotate = cube[array_to_rotate_coordinates_cube[0]][array_to_rotate_coordinates_cube[1]]
    length = len(array_to_rotate)
    for index_row in range(length):
        for index_column in range(index_row, length):
            temp = array_to_rotate[index_column][index_row]
            array_to_rotate[index_column][index_row] = array_to_rotate[index_row][index_column] # = element
            # that which get overwritten is indexcomuln,indexrow, but it doesnt matter because we already stored it in temp.
            array_to_rotate[index_row][index_column] = temp

    return array_to_rotate

def switch_columns(array_to_rotate_coordinates_cube):
    """Changes the position of the columns. the ones outside, with other ones outside. """

    array_to_order = cube[array_to_rotate_coordinates_cube[0]][array_to_rotate_coordinates_cube[1]]

    lenght = len(array_to_order)
    for index_row in range(0, lenght):
        for column_row in range(0, lenght//2):

            temp = array_to_order[index_row][column_row] # links
            array_to_order[index_row][column_row] = array_to_order[index_row][(lenght-1)-column_row] #rechts
            array_to_order[index_row][(lenght - 1) - column_row] = temp# oldlinks speichern wir in rechts
...
def Clockturn(Turning_face):
    ...
    transpose_matrix(Turning_face)
    switch_columns(Turning_face)
...
cubedict = {
    ...
    "scuffed":
        [[[['', '', ''], ['', '', ''], ['', '', '']], [['red', 'yellow', 'white'], ['orange', 'orange', 'yellow'], ['green', 'red', 'blue']], [['', '', ''], ['', '', ''], ['', '', '']], [['', '', ''], ['', '', ''], ['', '', '']]], [[['blue', 'white', 'red'], ['green', 'green', 'blue'], ['yellow', 'white', 'yellow']], [['white', 'blue', 'yellow'], ['white', 'white', 'orange'], ['green', 'blue', 'blue']], [['orange', 'green', 'orange'], ['yellow', 'blue', 'red'], ['red', 'white', 'white']], [['green', 'blue', 'yellow'], ['yellow', 'yellow', 'red'], ['orange', 'green', 'green']]], [[['', '', ''], ['', '', ''], ['', '', '']], [['red', 'orange', 'white'], ['red', 'red', 'green'], ['orange', 'orange', 'blue']], [['', '', ''], ['', '', ''], ['', '', '']], [['', '', ''], ['', '', ''], ['', '', '']]]]
}
